...
/**
 * This program lets the user edit short text files in a window. When a file is being edited, the name of the file
 * is displayed in the window's title bar. A "File" menu provides the following commands:
 *
 *               New -- Clears all text from the window.
 *               Open -- Lets the user select a file and loads up to 100 lines
 *                       of text from that file into the window. The previous contents of the
 *                       window are lost.
 *               Save -- Lets the user specify an output file and saves the contents of the window in that file.
 *               Quit -- Closes the window and ends the program.
 *
 * This is very simple program is not meant for serious text editing. In particular, a limit of
 * 10000 characters is put on the size of the files that it can read.
 */
public class FileEditor extends Application {
    ...
    private File editFile;  // The file, if any that is currently being edited. If non-nul, this file was selected
                            // by the user in a file open or file save dialog.
    private Stage mainWindow;  // The program window, used for changing the window title.


    /**
     * Open a window, with a TextArea where the user can edit some text and with a menu bar for
     * file operations.
     */
    ...
    /**
     * Create and return a menu bar containing a single menu, the File menu. This
     * menu contains four commands, New, Open, Save, and Quit.
     */
    private MenuBar makeMenuBar() {
        ...
        openCommand.setOnAction( e -> doOpen() );

        MenuItem saveCommand = new MenuItem( "Save" );
        saveCommand.setOnAction( e -> doSave() );

        fileMenu.getItems().addAll( newCommand, openCommand, saveCommand );
        fileMenu.getItems().add( new SeparatorMenuItem() );

        MenuItem quitCommand = new MenuItem( "Quit" );
        quitCommand.setOnAction( e -> doQuit() );
    ...
    /**
     * Carry out the "New" command from the File menu by clearing all the text from the TextArea.
     * Also sets the title bar of the window to read "Text Editor: Untitled".
     */
    private void doNew() {
        text.setText( "" );
        editFile = null;
        mainWindow.setTitle( "Text Editor: Untitled" );
    }

    /**
     * Carry out the Save command by letting the user specify an output file and writing
     * the text from the TextArea to that file.
     */
    private void doSave() {
        ...
        // Note: User has selected a file AND if the file exists has confirmed that it is OK
        ...
        try {
            ...
            mainWindow.setTitle( "Text Editor: " + editFile.getName() );
        }
        catch ( Exception e ) {
     ...
     * successfully and is not too long, then the text from the file replaces the text in the TextArea.
     */
    public void doOpen() {
        FileChooser fileDialog = new FileChooser();
